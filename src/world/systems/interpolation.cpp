...
namespace interpolation_systems {
    ...
    void register_systems(const World &world) {
        auto store_previous = [](const flecs::entity entity, const WorldTransform &transform) {
            auto& state = entity.ensure<InterpolationState>();
            state.prev_pos = transform.pos;
            state.prev_yaw = transform.yaw;
        };

        auto set_render_state = [](const flecs::iter &iter, size_t, const WorldTransform &transform, InterpolationState &state) {
